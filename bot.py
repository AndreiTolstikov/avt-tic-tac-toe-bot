# -*- coding: utf-8 -*-
#
# avtTicTacToeBot is a chatbot of Tic-Tac-Toe game for Telegram

# avtTicTacToeBot based on Simple Bot to reply to Telegram messages
# created as example for python-telegram-bot library, that provides
# a pure Python interface for the Telegram Bot API.
#
# Simple Bot program were created by jh0ker (Jannes Höke),
# jsmnbom (Jasmin Bom), tsnoam (Noam Meltzer), SitiSchu (Simon Schürrle),
# rahiel (Rahiel Kasim), bluehood (Enrico Guiraud)
#
# Simple Bot program is dedicated to the public domain under the CC0 license.
#
# Text and image wich opens with /rule command from avtTicTacToeBot
# was created by Al Sweigart, and  is licensed under the Creative Commons
...
Usage:
Example of a bot-user Tic-Tac-Toe game using ConversationHandler.
Send /start to start a game.
...
def play_again_msg(bot, update):
    """Send message: Do you want to play again? (yes or no)

    Arguments:
        bot {telegram.Bot} -- instance of the telegram.Bot
        update {telegram.ext.Updater} -- instance of the telegram.ext.Updater
    """
...
def bot_move(bot, update, board, bot_game_letter):
    """Given a board and the bot's game letter,
            determine where to move and make that move.
...
def analyze_bot_move(bot, update, board, bot_game_letter):
    """Analysis of the game situation after the bot's move
...
def start(bot, update):
    """Callback function using as entry point to start game
...
def init_game(bot, update):
    """Callback function for preparing game parameters
        and performing the first move by one of the opponents
...
def game_loop(bot, update):
    """Callback function for for preparing game parameters
    and performing the first move by one of the opponents
...
def game_rules(bot, update):
    """Send message with Tic-Tac-Toe game rules
...
def cancel(bot, update):
    ...
    Returns:
        [int] -- A constant to return when the game is ended
    """
